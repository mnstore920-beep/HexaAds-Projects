...
import React, { useState, useRef, KeyboardEvent, Suspense } from 'react';
...
function VerifyForm() {
  ...
  const isComplete = otp.every(digit => digit !== '') && newPassword.length >= 6;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Verification failed. Invalid or expired code.');
        setLoading(false);
        return;
      }
